...
export const researchTopicForPrompt = async (topic, level, style, language) => {
...
export const generateInfographicImage = async (prompt) => {
    const response = await getAi().models.generateContent({
        model: IMAGE_MODEL,
        contents: {
            parts: [{ text: prompt }]
...
export const editInfographicImage = async (currentImageBase64, editInstruction) => {
